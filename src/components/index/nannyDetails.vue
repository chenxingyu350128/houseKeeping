  ...
  <div class="nannyDetails white subtitle-2">
    ...
    >
      <template v-slot:right>
        <v-icon @click="changeCollectState">
          {{ isCollect?'mdi-star':'mdi-star-outline' }}
        </v-icon>
    ...
    <v-divider />
    <div
      v-if="!showMore&&labels.length"
      class="d-flex px-2 py-1 flex-wrap"
    >
      <v-btn 
    ...
    <v-divider />
    <div class="subtitle-1 px-4 py-2">
      基本信息
    </div>
    <v-divider />
    ...
    <v-divider />
    <div
      class="subtitle-2 px-4 py-2 my-2 d-flex align-center"
      @click="toShowEvals"
    >
    ...
    <div class="px-4 py-2 title font-weight-bold">
      相似阿姨推荐
    </div>
    <v-divider />
    <v-list 
...
export default {
    ...
    methods: {
        ...
        changeCollectState() {
            const url = this.isCollect ? '/nanny/delNannyCollect' : '/nanny/addNannyCollect'
            const data = {
                userId: this.userId
            }
            if (this.isCollect) {
