<template>
  <div class="payPage grey lighten-2">
    ...
    <v-divider />
    <div class="px-4 py-2 text-right mb-2 white">
      实付款：<span class="pink--text text--accent-2">￥{{ actualPayment }}</span>
    </div>
    <div class="px-4 py-2 white">
      支付方式
    </div>
    <div
      class="px-4 py-2 d-flex align-center justify-space-between white"
      @click="payWay=0"
    >
      <div class="d-flex align-center">
        <v-avatar
          ...
          size="25"
        >
          <v-img src="../assets/img/WeChat.png" />
        </v-avatar>
        微信支付
      </div>
      <v-icon color="primary">
        {{ payWay==0?'mdi-check-circle':'mdi-circle-outline' }}
      </v-icon>
    </div>
    <div
      class="px-4 py-2 d-flex align-center justify-space-between white"
    ...
    >
      <div class="d-flex align-center">
        <v-avatar
          ...
          size="25"
        >
          <v-img src="../assets/img/alipay.png" />
        </v-avatar>            
        支付宝支付
      </div>
      <v-icon color="primary">
        {{ payWay==1?'mdi-check-circle':'mdi-circle-outline' }}
      </v-icon>
    </div>
...
export default {
    ...
    methods: {
      ...
      async commit() {
        ...
        const res = await this.$http.post('/order/payAttestation', data)
        if (res.data.success) {
