  ...
  <div class="orderDetails grey lighten-3 subtitle-2">
    ...
    >
      <div class="ma-4">
        <!-- 进度 -->
        <v-stepper
          v-model="attemperStatus"
          class="teal white--text lighten-3"
        >
          <v-stepper-header
            style="height: 36px"
            height="30"
      ...
      <div class="white subtitle-2 px-4 py-2 my-2">
        ...
        >
          ...
          <v-icon
            color="primary"
            @click="toCall(auntModel.userName)"
          >
            mdi-phone-in-talk
          </v-icon>
        </div>
    ...
    >
      <div
        v-if="state!==''"
        :class="state==1||state==2?'white--text cyan lighten-2':'red--text' "
      ...
      <div
        v-if="cancelCause"
        class="white text--secondary mb-2"
      >
        <div class="px-4 py-2">
          订单取消原因：
        </div>
      ...
      <div class="px-4 py-2 white text--secondary caption">
        ...
        <div v-if="completeTime">
          完成时间：{{ completeTime }}
        </div>
        <div v-if="evaluateTime">
          评价时间：{{ evaluateTime }}
        </div>
      </div>
    </div>
    <v-footer
      class="pa-0 ma-0 d-flex flex-row-reverse"
      fixed
      bottom
    >
      <v-row class="pa-0 ma-0">
        ...
        >
          <v-btn
            ...
            block
            depressed
            @click="toPay(orderServices[0])"
          >
            去支付
          </v-btn>
        </v-col>
        <v-col
          v-if="state==3&&!evaluateTime"
          class="py-1"
          cols="6"
        >
          <v-btn
            ...
            block
            depressed
            @click="toEvaluate(orderServices[0])"
          >
            去评价
          </v-btn>
        </v-col>
        <v-col
          v-if="compModel&&compModel.servicePhone"
          class="py-1"
          cols="6"
        >
          <v-btn
            ...
            block
            depressed
            @click="toCall(compModel.servicePhone)"
          >
            联系客服
          </v-btn>
        </v-col>
    ...
    <evaluation
      ...
      :item-id="itemId"
      :nanny-id="nannyId"
      @evalSuccess="showEvaluate=false;init()"
...
export default {
    ...
    methods: {
      async init() {
        ...
        const res = await this.$http.get('/order/findOrderById', { params })
      ...
      toProductDetails() {
        const id = this.itemId || this.auntId || this.nannyId
        this.$router.push({
          path: '/productDetails',
          query: {
            id
